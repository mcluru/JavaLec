...
public class Course {
	...
	//매개변수인 cNo 와 멤버변수 cNo는 다른 변수.
	//매개변수 cNo는 생성자 함수를 호출했을 때 보내주는 값을 받기 위한 변수
	public Course(int cNo) {
		//this 객체 : 현재 속해있는 클래스를 지칭
		//매개변수로 받아온 cNo 과 멤버변수 cNo를 구분하기 위해 this 사용
		this.cNo = cNo;
	}
...
//		this(); //Course 기본생성자 호출
		
		//매개변수가 두 개인 생성자에서 cNo, cName 속성을 초기화해주기 때문에
		//현재 생성자에서는 따로 cNo, cName에 대한 초기화작업 불필요
